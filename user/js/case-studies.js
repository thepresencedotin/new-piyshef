...
firebase.firestore().collection("case-studies").orderBy("priority", "asc").onSnapshot(function (snapshot) {
    ...
    snapshot.forEach((element, index) => {
        document.getElementById("case-studies").innerHTML += `
        <figure class="effect-apollo px-auto" data-aos="fade-up" data-aos-duration="2000" style="max-width:45%;min-height:450px;max-height:450px;background-image:url(${element.data().imgUrl});background-size:cover;background-repeat: no-repeat;">
            <figcaption>
